    ...
    utils.lib.eachDefaultSystem (system:
      let
        ...
        rustToolchain = pkgs.rust-bin.stable.latest.minimal.override {
          extensions =
            [ "rust-src" "rust-analysis" "rustfmt-preview" "clippy" ];
        ...
        craneArgs =
          (craneLib.crateNameFromCargoToml { cargoToml = ./tf-ncl/Cargo.toml; })
          // {
            src = tf-ncl-src;
          };
        ...
        eval-github-yaml = pkgs.writeShellScript "eval-github-yaml" ''
          ...
          find .github -name '*.ncl' -print0 | while IFS= read -d $'\0' f; do
            (echo "# DO NOT EDIT! Generated by eval-github-yaml from $f"; \
              ...
              }/bin/nickel export --format json -f "$f") > "''${f%.ncl}.yml"
          done
        '';

        pre-commit = inputs.pre-commit-hooks.lib.${system}.run {
          src = ./.;
          tools = { inherit (pkgs) cargo rustfmt; };
          hooks = {
        ...
        release = pkgs.runCommand "release-tarball" {
          ...
          mkdir tf-ncl
          ${lib.concatLines
          (lib.flip lib.mapAttrsToList inputs.self.schemas.${system}
            (provider: schema: ''
        ...
        test-single-example = template:
          pkgs.writeShellScript "test-${template}" ''
            set -e -x
        ...
        test-examples = pkgs.writeShellScriptBin "test-examples" ''
          set -e -x
          ${lib.concatMapStringsSep "\n" (tmpl: "${test-single-example tmpl}")
          (lib.attrNames self.templates)}
        '';
      in {
        checks = self.schemas.${system} // (lib.mapAttrs' (name: drv:
          lib.nameValuePair "check-${name}" (let
        ...
        packages = {
          ...
          (name: value: lib.nameValuePair "schema-${name}" value)
          self.schemas.${system};

        inherit terraformProviders;

        generateJsonSchema = providerFn:
          pkgs.callPackage
          (import ./nix/terraform_schema.nix (providerFn terraformProviders)) {
            inherit (self.packages.${system}) schema-merge;
          };

        generateSchema = providerFn:
          pkgs.callPackage ./nix/nickel_schema.nix {
            jsonSchema = self.generateJsonSchema.${system} providerFn;
            inherit (self.packages.${system}) tf-ncl;
          };
